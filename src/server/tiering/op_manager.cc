  ...
  for (size_t i = 0; i < info->key_ops.size(); i++) {
    ...
    if (page) {
      std::string_view substr =
          page->substr(ko.segment.offset - info->segment.offset, ko.segment.length);
      ko.decoder->Initialize(substr);
    ...
    if (page.has_value() && !delete_from_storage)
      delete_from_storage |= NotifyFetched(Borrowed(ko.id), ko.segment, std::move(*ko.decoder));
...
OpManager::EntryOps::EntryOps(OwnedEntryId id, DiskSegment segment, const Decoder& decoder)
    : id{std::move(id)}, segment{segment}, decoder{decoder.Promote()} {
}
...
OpManager::EntryOps& OpManager::ReadOp::ForSegment(DiskSegment key_segment, EntryId id,
  ...
  for (auto& ops : key_ops) {
    if (ops.segment.offset == key_segment.offset) {
      DCHECK(typeid(decoder) == typeid(*ops.decoder));  // Decoders must be of same type
      return ops;
    }
  }
